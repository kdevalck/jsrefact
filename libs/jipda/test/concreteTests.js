...
(function () 
{
  var module = new TestSuite("concreteTests");
  ...
  function run(src, expected)
  {
    ...
    var actual = state.result;
    assertEquals(ipda.lattice.abst1(expected), actual);    
  }
  ...
  function runStr(src, expected)
  {
    ...
    var actual = state.result.accept(printer);
  ...
  module.test52a =
    function ()
    {
      run("var Circle=function (radius) {this.radius = radius;}; Circle.prototype;", ({}));
      run("function Circle(radius) {this.radius = radius;}; Circle.prototype;", ({}));
